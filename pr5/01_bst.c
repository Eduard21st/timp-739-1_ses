...
void clean2(node *n)//4 5 2 3 7 6 1 1 4 3
{
...
void clean(tree *t,node *n)//4 5 2 3 7 6 1 1 4 3
{
    if(t->root == NULL)
        return;
    clean2(n);
...
// Выполняет левый поворот дерева относительно корня
...
// Выполняет правый поворот дерева относительно корня
...
//выводит предка и потомков элемента
void print(node * n)
{
    if(n == NULL)
    {
